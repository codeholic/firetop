...
$Home = shift
  or die "Need to supply \$Home as first argument to process.pl\n";
...
%GestureDesc = (
    'F', 'wriggles the fingers of', 'P', 'proffers the palm of',
    'S', 'snaps the fingers of',    'W', 'waves',
    'D', 'points the digit of',     'C', 'claps with',
    '>', 'stabs with',              '-', 'does nothing with'
);
...
%HandMap = ( 'Right', 'RH', 'Left', 'LH', 'Both', 'BH', 'Fire', 'Fire' );
...
ORDER: while (1) {
        ...
        && ( $passwd = $1, crypt( $passwd, "aa" ) eq "aae.ep8N3noK2" ) )
    {
        ...
        print MAIL
          "Subject: Firetop Mountain Server has been turned OFF\n\nOK\n";
        close MAIL;
        close LOG;
        flock( LOCK, 8 );
        close(LOCK);
...
"\n > Please use alpha-numeric characters only \n  - other characters are too hard to pronounce!\n";
    ...
    if ( $keyword eq "USER" ) {
        ...
        if ($Referee) {
            ...
            if ( $plrAddr ne $gmAddr
                && !grep( $plrAddr eq $_, @MaintainerAddresses ) )
            {
                $parse .=
...
"\n You only successfully specified the gesture of one hand!\n";
                $parse .=
                  " Please try again! (You will need to resubmit both hands)\n";
                $errs++;
            }
        }
        last;
...
"\n > *** DATABASE ERROR: Janitor has been notified.  Please don't submit more orders for this game or wizard. ***\n";
        ...
        $SubjectLine =
          "Subject: Battle $GameName, turn $Turn (Results of Orders)\n\n";
...
"\n You can not make announcements in games you are not participating in.\n";
                    print LOG
                      "False annoucement from $plrAddr to game $gamenum\n";
                    $errs++;
                    last;
                }
            }
...
"\n > Can't process this type of command with ordinary game orders.\n";
        ...
        || ( $keyword eq "DECLINE" ) )
    {
        ...
        if ($NewGame) {
            if ( -f "$NewGame.ngm" ) {
                $SubjectLine =
"Subject: Challenge $NewGame on Firetop Mountain (Results of Orders)\n\n";
            }
            else {
...
"\n > $keyword Can't process ordinary orders with previous command(s)\n > Ignoring the rest of the mail.\n";
    ...
    if ( $keyword eq "TARGET" ) {
        ...
        if (   !( &Upcase( $arg[0] ) =~ m/^((RH)|(LH)|(BH))$/ )
            && !&IsMonsterInGame( $arg[0] ) )
        {
...
if ( $gmAddr eq $plrAddr ) {
    ...
    open( MAIL, ">>$LogDir/to.$gmAddr.log" )
      || die "Could not open $LogDir/to.$gmAddr.log\n$!\n";
}
else {
    &OpenMail( MAIL, $plrAddr );
}
...
";
        ...
        while (<RULES>) {
            if ( $arg[0] and ( $line_number % $arg[0] == 0 ) ) {
                ...
                open( RULE_MAIL,
                    "|$sendmail -f$gmAddr -F\"$gmName\" $plrAddr" );
                print RULE_MAIL
"Subject: Rules of Combat on Firetop Mountain (Part $part_number)\n\n";
            }
    ...
    if ( $keyword eq "FAQ" ) {
        ...
        print FAQ_MAIL
          "Subject: Firetop Mountain Frequently Asked Questions\n\n";
    ...
    if ( $keyword eq "NEWUSER" ) {
        ...
        print MAIL
          "$NewUser has been registered as a user at Firetop Mountain.\n";
 ...
 #        print "$NewUser has been registered as a user at Firetop Mountain.\n";
    ...
    if ( $keyword eq "RESEND" ) {
        ...
        if ( !open( REPORTS, "$LogFileName" ) ) {
            print MAIL
              "There seems to be no record of reports to send to you!\n";
            next;
        }
...
"Subject: Resend of Firetop Mountain results ($Num of $arg[0])\n\n";
    ...
    if ( $keyword eq "REGISTER" ) {
        ...
        if ( $Wizard{$Name}{'User'} ) {
            ...
            print MAIL
"An imposter posing as $Name was turned away from Firetop Mountain!\n";
            next COMMAND;
        }
...
"An imposter posing as $Name was turned away from Firetop Mountain!\n";
        ...
        if ( &IsMonster($Name) ) {
            ...
            print MAIL
"A Monster posing as a Wizard was turned away from Firetop Mountain!\n";
            next;
        }

        if ( $Name =~ m/janitor/i ) {
            print MAIL
"$h The Firetop Mountain Janitor sidles over and looks you up and down...\n";
            print MAIL
"\"There's only room for one Janitor on this Mountain, and it ain't you, punk!\",\n";
            print MAIL
              " he says, and suddenly you find yourself a long way away...\n";
            next;
        }
...
to play again you are always welcome to register again as a new user.\n";
        ...
        if ( $Wizard{$Name}{'Busy'} ) {
            ...
            if ( $game =~ s/^N(.*)$/$1/ ) {
                print MAIL "You cannot "
                  . "\L$keyword"
                  . " now, when you have accpted challenge $game!\n";
            }
            else {
                print MAIL
                  "You'll need to finish your battle $game before you can "
                  . "\L$keyword" . "\n";
            }
            next;
        }
          ...
          )
        {
            ...
            print MAIL
"$Name retires greacefully and goes to sit with the other mages resting\nin the shade of the great Menhirs surrounding the Circle.\n";
            &NoteActivity($Name);
        }
        else {
            print MAIL
"$Name leaves Firetop Mountain, having survived the experience, but\nnot ready to battle further at this stage.\n";
            $Wizard{$Name}{'Dead'} = 1;
        }
        $ShowScores = "all";
        ...
        while ( $GN = shift @arg ) {
            if ( $GN =~ s/^N// ) {
                ...
                if ( grep( $_ eq $User, @Subscribers ) ) {
                    print MAIL
"You are already among those that will receive updates on game $GN.\n";
                }
                else {
                ...
                if ( grep( $_ eq $User, ( @Subscribers, @Players ) ) ) {
                    print MAIL
                      "You are already reciving updates on game $GN.\n";
                }
                else {
                    push( @Subscribers, $User );
                    &WriteGameInfo($GN);
...
";
    ...
    if ( $keyword eq "LH" || $keyword eq "RH" ) {
        ...
        if (
            length( $Info{$Player}{$keyword} ) >
            length( $Info{$Player}{$OtherHand} ) )
        {
        ...
        @RightSpells =
          &CheckForCastRight( $Info{$Player}{'RH'}, $Info{$Player}{'LH'} );
        @LeftSpells =
          &CheckForCastLeft( $Info{$Player}{'RH'}, $Info{$Player}{'LH'} );
        ...
        if ( $Info{$Player}{'LeftSpell'} !~ /^choose$|^none$/ ) {
            $Event .=
"You prepare to cast $Info{$Player}{'LeftSpell'} with your left hand.\n\n";
        }
        if ( $Info{$Player}{'RightSpell'} !~ /^choose$|^none$/ ) {
            $Event .=
"You prepare to cast $Info{$Player}{'RightSpell'} with your right hand.\n\n";
        }
        if ( $Info{$Player}{'BothSpell'} !~ /^choose$|^none$/ ) {
            $Event .=
"You prepare to cast $Info{$Player}{'BothSpell'} with both of your hands.\n\n";
        }
    ...
    if ( $keyword =~ m/^CHO/ ) {
        ...
        if ( $Info{$Player}{'State'} ne "choose-spell" ) {
            if ( $Hand eq "RH" ) {
                print MAIL
"Your Right Hand spell is '$Info{$Player}{'RightSpell'}' - you have no choice.\n";
            }
            if ( $Hand eq "LH" ) {
                print MAIL
"Your Left Hand spell is '$Info{$Player}{'LeftSpell'}' - you have no choice.\n";
            }
            if ( $Hand eq "BH" ) {
                print MAIL
"Your Both-Hands spell is '$Info{$Player}{'BothSpell'}' - you have no choice.\n";
            }
            next;
        }
        ...
        if ( $Hand eq "RH" ) {
            if ( $Info{$Player}{'RightSpell'} ne 'choose' ) {
                print MAIL
"You have no choice about your Right Hand spell: '$Info{$Player}{'RightSpell'}'.\n";
                next;
            }
            ...
            if ( $Info{$Player}{'LeftSpell'} ne 'choose' ) {
                print MAIL
"You have no choice about your Left Hand spell: '$Info{$Player}{'LeftSpell'}'.\n";
                next;
            }
            ...
            if ( $Info{$Player}{'BothSpell'} ne 'choose' ) {
                print MAIL
"You have no choice about your Both Hands spell: '$Info{$Player}{'BothSpell'}'.\n";
            }
            ...
            foreach $spell (@BothSpells) {
                if ( &Upcase($spell) eq &Upcase($Choice) ) {
                    ...
                    $GotChoice                   = 1;
                    print MAIL
                      "You chose $Choice for your both-hands spells.\n";
                }
            }
        }
    ...
    if ( $keyword eq "TARGET" ) {
        ...
        $targ = ( ( $t eq $Player ) ? "yourself" : &BeingDescription($t) );
        $targ =~ s/$Player is\b/you are/g;

        if (   grep ( $targ eq $_, @DeadMonsters )
            && !$RaiseDead
            && !( $targ =~ /(Fire|Ice)Elemental/
                && ${ $1 . "ElementalPresent" } ) )
        {
        ...
        if ( $Hand eq "RH" ) {
            ...
            if ( $Info{$Player}{'Target'} eq 'no_one' ) {
                ...
                $Info{$Player}{'Target'} =~
                  s/(^|<)$Monster=>[^<]*/$1$Monster=>$t/
                  or $Info{$Player}{'Target'} .= "<$Monster=>$t";
            }
    ...
    if ( $keyword eq "PARALYZE" ) {
        ...
        {
            print MAIL "$h One of $t"
              . "'s hands is already stuck (and will be again this turn)!\n";
            next;
        }
    ...
    if ( $keyword eq "DIRECT" ) {
        ...
        if ( $Info{$Victim}{'CharmedRight'} || $Info{$Victim}{'CharmedLeft'} ) {
            print MAIL "You have already directed one of $Victim"
              . "'s hands.\n";
            next;
        }
    ...
    if ( $keyword eq "PERMANENT" ) {
        ...
        if ( !$Info{$Player}{'PermanencyPending'} ) {
            print MAIL
              "$h You don't have the capability for that at the moment.\n";
            next;
        }

        print MAIL
          "You apply your reserve of power to make a spell permanent.\n";
            ...
            foreach $hand ( 'Right', 'Left', 'Both' ) {
                    ...
                    && $Info{$Player}{ $hand . 'Target' } eq 'none' )
                {
                    ...
                    if ( !&SelfTarget($spell) && !&UntargettedSpell($spell) ) {
                        print MAIL
"There is no default target for your $hand hand spell ($spell).\n\n";
                        print MAIL
"You may target the spell before the round continues.\n\n";
                        $Info{$Player}{'State'} = 'choose-target';
                    ...
                    && ( $Info{$Player}{"${hand}Target"} eq 'none' ) )
                {
                    ...
                    print MAIL
"You may target your \L$hand hand before the round continues.\n\n";
                    $Info{$Player}{'State'} = 'choose-target';
                    $Info{$Player}{ $hand . 'Target' } = 'no_one';
                }
            }
            ...
            if ( !&AllOrdersIn ) {
                if ($NewSpellChoises) {
                    if ( $NewSpellChoises > 1 ) {
                        &Announce(
"Enchantments have resulted in wizards having a new choice of spells\n"
                              . "The round will resume when these choices have been made...\n\n"
                        );
                    }
                    else {
                        &Announce(
"An enchantment has resulted in a wizard having a new choice of spells\n"
                    ...
                    if ( $NewTargets > 1 ) {
                        &Announce(
"Enchantments have resulted in wizards having a new chance to target spells.\n"
                              . "The round will resume when these targets have been set...\n\n"
                        );
                    }
                    else {
                        &Announce(
"An enchantment has resulted in a wizard having a new chance to target spells.\n"
        ...
        foreach $ToUser ( $User, @OpponentUsers ) {
            ...
            for $i ( 0 .. $#Events ) {
                ...
                if (@Lookers) {
                    if (
                        grep( substr( $Info{$_}{'SeeEvent'}, $i, 1 ), @Lookers )
                      )
                    {
                        $SeeEvent = 1;
                    }
                    ...
                    $SeeEvent = 1;
                    if (
                        grep( 0 == substr( $Info{$_}{'SeeEvent'}, $i, 1 ),
                            @Players )
                      )
                    {
                        $SeeEvent = 0;
...
print MAIL &Scores("Duel")
  if ( $ShowScores =~ m/duel/i
    || $ShowScores =~ m/all/i );
print MAIL &Scores("Melee")
  if ( $ShowScores =~ m/melee/i
    || $ShowScores =~ m/all/i );
...
sub PrepareForTurn {
    ...
    foreach $wiz (@Players) {
        ...
        if ( $Info{$wiz}{'CheckNewSpells'} ) {
            ...
            @BothSpells =
              &CheckForCastBoth( $Info{$wiz}{'RH'}, $Info{$wiz}{'LH'}, $wiz );
...
"\nDue to an enchantment, your spells have changed this round:\n";
                    print $ChooserMail $spells;
                    print $ChooserMail
                      "You will also stab with your $newstab.\n"
                      if $newstab;
                    print $ChooserMail
"If you wish to change the target of the spell(s), you may do so now.\n"
    ...
    foreach $wiz (@Players) {
        ...
        $Info{$wiz}{'BlindTurns'} .= ( $Info{$wiz}{'Blind'} ? '1' : '0' );
    ...
    foreach $wiz (@Players) {
        next if grep( /^$wiz$/, @DeadPlayers );
        next
          if ( ( $TimeStoppedTurn && !$Info{$wiz}{'TimeStoppedTurn'} )
    ...
    foreach $wiz (@Players) {
        ...
        if ( $Info{$wiz}{'Forgetful'} ) {
            if ($HastenedTurn) {
                &NoteThat(
"$wiz is slowly getting dopey, but $wiz does still manage to remember what to do.\n",
                    $wiz
                );
            }
            elsif ($TimeStoppedTurn) {
                &NoteThat(
"Escaping briefly from amnesia, $wiz manages to remember what to do.\n",
                    $wiz
                );
            }
            else {
                &NoteThat(
                    "Looking dopey, $wiz repeats last round's gestures!\n",
                    $wiz );
            }
        }
            ...
            if ($HastenedTurn) {
                &NoteThat(
"Speedily $wiz makes some new gestures before getting too afraid.\n",
                    $wiz
                );
            }
            elsif ($TimeStoppedTurn) {
                &NoteThat(
"From another time line, things seem not so frightening to $wiz.\n",
                    $wiz
                );
            }
        }
            ...
            if ($HastenedTurn) {
                &NoteThat(
"Speedily $wiz makes some new gestures before getting too confused.\n",
                    $wiz
                );
            }
            elsif ($TimeStoppedTurn) {
                &NoteThat(
                    "From another time line, things seem clearer to $wiz.\n",
                    $wiz );
            }
            else {
                ...
                if ( $Info{$wiz}{'Confused'} =~ s/Lucky// ) {
                    $Event .=
"By sheer luck, $wiz makes the intended gesture anyway!\n";
                }
                else {
            ...
            if ($HastenedTurn) {
                &NoteThat(
"Speedily $wiz makes some new gestures\nbefore the paralysis takes effect.\n",
                    $wiz
                );
            }
            elsif ($TimeStoppedTurn) {
                &NoteThat(
"In another timeline, $wiz finds that paralysis has not quite set in.\n",
                    $wiz
                );
            }
            else {
                foreach $hand ( 'Right', 'Left' ) {
                    if ( $Info{$wiz}{"Paralyzed$hand"} ) {
                        ...
                        if ( $Info{$wiz}{"Paralyzed$hand"} =~ s/Lucky// ) {
                            &NoteThat(
"Luckily, $wiz intended to repeat the gesture anyhow!\n",
                                $wiz
                            );
                        }
                    }
...
"Speedily $wiz makes some new gestures\nbefore losing control of one hand.\n",
                ...
                &NoteThat(
                    "From another timeline, $wiz finds "
                      . $Info{$wiz}{'Controller'}
                      . " a lot less charming.\n",
                    $wiz
                );
                ...
                foreach $hand ( 'Right', 'Left' ) {
                    ...
                    if ( $Info{$wiz}{"Charmed$hand"} ) {
                        ...
                        &NoteThat(
                            "$wiz"
                              . "'s $hand Hand is under the control of someone else!\n",
                            $wiz
                        );
                        if ( $Info{$wiz}{"Charmed$hand"} =~ s/Lucky// ) {
                            &NoteThat(
"Luckily, $wiz gets to do the desired gesture anyhow!\n",
                                $wiz
                            );
                        }
                    }
            ...
            if ( !$HastenedTurn && $Info{$wiz}{'TimeStoppedTurn'} ) {
                &Announce(
"$wiz sneaks in another gesture from a parallel time line...\n"
                );
            }
        }
        ...
        foreach $hand ( 'Right', 'Left', 'Both', 'Fire' ) {
            if ( $Info{$wiz}{ $hand . 'Spell' } ne "none" ) {
                ...
                if ( $Info{$wiz}{ $HandMap{$hand} . 'Save' } ) {
                    ...
                    $SpellDuration =
                      $Info{$wiz}{ $HandMap{$hand} . 'Duration' };
                    $Info{$wiz}{ $HandMap{$hand} . 'Duration' } = 1;
                    if ( $SpellDuration == 999 ) {
                        $Event .=
"${wiz}'s blue halo flares brightly as $wiz casts $SelectedSpell!\n";
                    }
        ...
        if ( $Info{$wiz}{'RHDuration'} + $Info{$wiz}{'LHDuration'} > 990 ) {
            &NoteThat(
                "The blue halo above $wiz flares brightly, then disappears.\n",
                $wiz
            );
        }
    }
    ...
    foreach $spellnumber ( 0 .. $#SpellOrder ) {
        ...
        foreach $SpellLine (@SpellsCast) {
            ...
            if ( $SelectedSpell eq $spell ) {
                $Event =
                  "";    #Clear this variable, in case $SpellTarget eq "no_one"
                if ( ( $SpellTarget ne "no_one" )
                    || &UntargettedSpell($SelectedSpell) )
                {
                    $Event = &CastSpell(
                        $SelectedSpell, $wiz, $SpellTarget,
                        $hand,          $SpellDuration
                    );
                }
    ...
    if ($DispelMagicInForce) {
        ...
        foreach $being (@Beings) {
            ...
            if ( &IsMonster($being) ) {
                $Event .=
                  "$being shrivels under the impact of the shock wave!\n";
                $Info{$being}{'HP'} = 0;
            }
            else {
    ...
    foreach $being (@Beings) {
        ...
        if ( &IsMonster($being) && $Info{$being}{'Charmed'} ) {
            ...
            &NoteThat(
                "$being looks to $Info{$being}{'Controller'} for guidance.\n",
                $being );
        }
        ...
        if ( $Info{$being}{'HitByFireBall'} ) {
            if ( $IceStormActive && !$TimeStoppedTurn ) {
                &NoteThat(
"The Fireball warms $being nicely as the Ice Storm rages all around.\n",
                    $being
                );
            }
                ...
                while ( $Info{$being}{'HitByFireBall'} ) {
                    if ( $being =~ m/FireElemental/ ) {
                        &NoteThat(
"The FireElemental is strengthent a moment as it absorbs a Fireball\n",
                            $being
                        );
                    }
                    else {
        ...
        while ( $Info{$being}{'HitByMissile'} ) {
                ...
                && $Info{$being}{'Shielded'} )
            {
                if ( $Info{$being}{'Countering'} ) {
                    &NoteThat(
"The hazy glow around $being effortlessly absorbs a Magic Missile.\n",
                        $being
                    );
                }
                else {
                    &NoteThat(
"A Magic Missile thunks into the glimmering shield protecting $being.\n",
                        $being
                    );
                }
            }
        ...
        while ( $Info{$being}{'Risen'} ) {
            if ( $Info{$being}{'Dead'} ) {
                &NoteThat(
"$being\'s new life energy saves $being from the Finger of Death\n",
                    $being
                );
                $Info{$being}{'Dead'}  = 0;
                $Info{$being}{'Risen'} = 0;
    ...
    foreach $wiz (@Players) {
        ...
        foreach $Command (@TargetCommands) {
            ...
            if (   ( $Monster eq 'null-monster' )
                || !&IsBeing($Monster)
                || grep( /^$Monster$/, @DeadPlayers ) )
            {
                ...
                if (    $Info{$Monster}{'Controller'}
                    and $Info{$Monster}{'Controller'} eq $wiz )
                {
                    if ( &IsBeing($Target) ) {
                    ...
                    $Event =
"$Monster looks in scorn at $wiz trying to take charge.\n";
                }
            }
            NoteThat( $Event, $wiz );
        }
    ...
    foreach $being (@Beings) {
        ...
        if ( $being =~ m/IceElemental/ ) {
            if ( $Info{$being}{'Confused'} ) {
                &NoteThat(
"It takes more than a bit of confusion to distract an angry Ice Elemental!\n",
                    $being
                );
            ...
            if ( $Info{$being}{'Afraid'} ) {
                &NoteThat(
"A taste of fear only makes the Ice Elemental more angry!\n",
                    $being
                );
                $Info{$being}{'Afraid'} = 0;
            }
                ...
                || $Info{$being}{'Forgetful'} )
            {
                ...
                $Info{$being}{'Forgetful'} =
                  Min( $Info{$being}{'Forgetful'}, 0 );
                $Info{$being}{'Paralyzed'} =
                  Min( $Info{$being}{'Paralyzed'}, 0 );
            ...
            foreach $target (@Beings) {
                ...
                if ( $Info{$target}{'Invisible'} ) {
                    ...
                    &NoteThat(
                        "$target shrugs off the Ice Elemental's assault.\n",
                        $target );
                }
                elsif ( $Info{$target}{'Shielded'} ) {
                    $ProtectString = &ProtectEffect($target);
                    &NoteThat(
"The Ice Elemental is deflected by $target\'s $ProtectString.\n",
                        $target
                    );
                }
                else {
                    &NoteThat(
                        "$target freezes under the Ice Elemental's assault!\n",
                        $target
                    );
            ...
            if ( $Info{$being}{'Confused'} ) {
                &NoteThat(
"It takes more than a bit of confusion to distract an angry Fire Elemental!\n",
                    $being
                );
                $Info{$being}{'Confused'} = 0;
            }
            if ( $Info{$being}{'Afraid'} ) {
                &NoteThat(
"A taste of fear only makes the Fire Elemental more angry!\n",
                    $being
                );
                $Info{$being}{'Afraid'} = 0;
            }
            ...
            foreach $target (@Beings) {
                ...
                if ( $Info{$target}{'Invisible'} ) {
                    ...
                    &NoteThat(
                        "$target shrugs off the Fire Elemental's assault.\n",
                        $target );
                }
                elsif ( $Info{$target}{'Shielded'} ) {
                    $ProtectString = &ProtectEffect($target);
                    &NoteThat(
"The Fire Elemental is deflected by $target\'s $ProtectString.\n",
                        $target
                    );
                }
                else {
                    &NoteThat(
                        "$target roasts under the Fire Elemental's assault!\n",
                        $target
                    );
            ...
            if ( $target eq 'no_one' ) {
                ...
                &NoteThat(
"$being wanders around looking for someone called $target.\n",
                    $being
                );
            }
            elsif ( $Info{$target}{'Invisible'} ) {
                &NoteThat(
                    "$being rushes around angrily trying to find $target.\n",
                    $being );
            }
            elsif ( $Info{$target}{'Shielded'} ) {
                $ProtectString = &ProtectEffect($target);
                &NoteThat(
                    "$being is deflected by $target" . "'s $ProtectString.\n",
                    $being );
            }
            else {
          ...
          STAB_CHECK:
            foreach $hand ( 'Right', 'Left' ) {
                ...
                if ( $Info{$being}{"LastGesture$h"} eq '>' ) {
                    ...
                    if ( $target eq 'no_one' ) {
                        &NoteThat(
"With dagger clenched in \L$hand\E hand, $being stabs aimlessly.\n",
                            $being
                        );
                    }
                    elsif ( !&IsBeing($target)
                        || grep( /^$target$/, @DeadPlayers ) )
                    {
                        &NoteThat(
"With dagger clenched in \L$hand\E hand, $being attempts to stab someone called $target.\n",
                            $being
                        );
                    }
                    elsif (
                    ...
                    {
                        &NoteThat(
                            "$being stabs wildly, hoping to hit $target.\n",
                            $being );
                    }
                    elsif ( $Info{$target}{'Shielded'}
                        && !$Info{$being}{'TimeStoppedTurn'} )
                    {
                        ...
                        &NoteThat(
                            "$being"
                              . "'s \L$hand\E stab is deflected by $target"
                              . "'s $ProtectString.\n",
                            $being
                        );
                    }
        ...
        if ( $being =~ m/(Ice|Fire)Elemental/ ) {
            ...
            foreach $target (@Beings) {
                ...
                if ( $Info{$being}{'TimeStopped'} ) {
                    ...
                    {
                        &NoteThat(
"Then, from another timeline, $being manages to hit $target after all!\n",
                            $target
                        );
                    }
                    else {
                        if ( $Element eq 'Ice' ) {
                            &NoteThat(
"Then, from another timeline, $being freezes $target again!\n",
                                $target
                            );
                        }
                        else {
                            &NoteThat(
"Then, from another timeline, $being roasts $target again!\n",
                                $target
                            );
                        }
                    }
                ...
                {
                    &NoteThat(
                        "Being hastened, the $being hits $target again!\n",
                        $target );
                    $Info{$target}{'HP'} -= $Damage{$being};
                }
            }
        ...
        else    # must be monster...
        {
            ...
            if ( $Info{$being}{'TimeStopped'} ) {
                ...
                {
                    &NoteThat(
"Then, from another timeline, $being manages to hit $target afterall!\n",
                        $target
                    );
                }
                else {
                    &NoteThat(
"Then, from another timeline, $being manages to hit $target again!\n",
                        $target
                    );
                }
                $Info{$target}{'HP'} -= $damage;
    ...
    foreach $being (@CurrentBeings) {
        ...
        if ( $Info{$being}{'Dead'} ) {
            if ( $Info{$being}{'NewRisen'} ) {
                &NoteThat(
"$being returns to the grave again, never being fully alive.\n",
                    $being
                );
            }
            if ( $Info{$being}{'TimeStoped'} ) {
                &NoteThat(
"$being\'s body flickers out for a moment, before falling stone dead.\n",
                    $being
                );
            }
            if ( $Info{$being}{'Fast'} ) {
                &NoteThat(
"$being was not fast enough to escape the Finger of Death!\n",
                    $being
                );
            }
        }
        ...
        if ( $Info{$wiz}{'HP'} > 0 ) {
            ...
            if ( $#Players > 1 ) {
                if ( $Info{$wiz}{'TimeStoped'} ) {
                    &NoteThat(
"$wiz\'s body flickers out for a moment, before falling stone dead.\n",
                        $wiz
                    );
                }
                if ( $Info{$wiz}{'Fast'} ) {
    ...
    if ( $#alive == -1 ) {
        ...
        &Announce(
            "What a battle!  The contestants have wiped each other out!\n");
        &ScoreGame();
        $ShowScores = $GameType;
        $FinishGame = 1;
    }
        ...
        if ( $#Players == 1 ) {
            ...
            &Announce(
                "$alive[0] surrendered to $deadguy even as $deadguy died!\n");
        }
        else {
        ...
        if ( $#Players == 1 ) {
            ...
            &Announce(
                "$alive[0] stands alone in the Graven Circle, Victorious!\n");
        }
        &ScoreGame(@alive);
        $ShowScores = $GameType;
        $FinishGame = 1;
        ...
        foreach $wiz (@Players) {
            ...
            if (   $Info{$wiz}{'State'} eq 'OUT'
                or $Info{$wiz}{'State'} eq 'Dead' )
            {
                $Info{$wiz}{'RH'} .= '-';
                $Info{$wiz}{'LH'} .= '-';
            ...
            if ( $#MageHastened >= 0 && !( $HastenedTurn || $TimeStoppedTurn ) )
            {
                if ( $Info{$wiz}{'Fast'} ) {
                    ...
                    foreach $opp (@MageHastened) {
                        &NoteThat(
"$wiz blinks in amazement while $opp moves in a blur...\n",
                            $wiz
                        );
                    }
...
"$wiz sneaks in another gesture from a parallel time line...\n",
                    ...
                    foreach $opp (@MageTimeStopped) {
                        &NoteThat(
"$wiz blinks as $opp winks out of existence for a moment!\n",
                            $wiz
                        );
                    }
                    $Info{$wiz}{'State'} = 'orders_in';
        ...
        foreach $wiz (@Players) {
            ...
            if (
                   ( $Info{$wiz}{'State'} eq 'orders' )
                && $Info{$wiz}{'Forgetful'}
                && !(
                       $Info{$wiz}{'HastenedTurn'}
                    || $Info{$wiz}{'TimeStoppedTurn'}
                )
              )
            {
                &NoteThat(
                    "$wiz"
                      . "'s mind is a complete blank!\n\n"
                ...
                @RightSpells =
                  &CheckForCastRight( $Info{$wiz}{'RH'}, $Info{$wiz}{'LH'} );
                @LeftSpells =
                  &CheckForCastLeft( $Info{$wiz}{'RH'}, $Info{$wiz}{'LH'} );
                @BothSpells =
                  &CheckForCastBoth( $Info{$wiz}{'RH'}, $Info{$wiz}{'LH'},
                    $wiz );
        ...
        foreach $being (@Beings) {
            ...
            if ( !$HastenedNextTurn && !$TimeStoppedTurn ) {
                foreach $atr (
                    'Blind',             'Countering',
                    'Fast',              'Invisible',
                    'PermanencyPending', 'DelayPending',
                    'Reflecting',        'Shielded'
                  )
                {
