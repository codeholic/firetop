...
$Home = shift
  or die "Need to supply \$Home as first argument to cleanup.pl\n";
...
if ( -f $LogFile and -M $LogFile < 0.5 ) {
    die
"Hey!  $LogFile has changed in under a day.  This doesn't seem right - wanna check your cron?\n";
}
...
!system("tar czf ../saves.backup.tar.gz *")
  or warn "tar job returned non-zero exit status!\n";
...
foreach $GameType ( 'Duel', 'Melee' ) {
    system(
        "mv $GameType.history.dat $GameType.history.dat.old;
          ...
          WIZ_CHECK:
            ...
            if ( -M $GameFile < $AutoResignTime && !$KillemAll ) {
                foreach $Culprit (@BadGuys) {
                    ...
                    &OpenMail( CULPRIT,
                        $Users{ $Wizard{$Culprit}{'User'} }{'Address'} );
                    print CULPRIT
                      "Subject: Reminder: Firetop Mountain Game $GameName\n\n";
                ...
                foreach $Culprit (@BadGuys) {
                    ...
                    &OpenMail( CULPRIT,
                        $Users{ $Wizard{$Culprit}{'User'} }{'Address'} );
                    print CULPRIT
"Subject: Final Notice: Firetop Mountain Game $GameName\n\n";
                ...
                if ( $#BadGuys == $#DeadBadGuys && @GoodGuys <= 1 ) {
                    $GoodMessage .=
"Congratulations $GoodGuys[0]!  You win due to your opponent's forfeit!\n\n"
                      if (@GoodGuys);
   ...
   #		    }
            ...
            foreach $GoodGuy (@GoodGuys) {
                &OpenMail( GOOD_GUY,
                    $Users{ $Wizard{$GoodGuy}{'User'} }{'Address'} );
    ...
    foreach $Wiz (@WizardNames) {
        if (    $Wizard{$Wiz}{'Retired'}
            and $Wizard{$Wiz}{'DuelScore'} + $Wizard{$Wiz}{'MeleeScore'} >
            $RetireThreshold )
        {
            $RetireThreshold =
  ...
  USER_CHECK:
    foreach my $User (@UserNames) {
        ...
        if ( !defined($UserActive) ) {
            print
              "Hmmm, $User isn't even in the stats list: adding $User now...\n";
              ...
              WIZ_CHECK:
                foreach my $Wiz (@UsersWizzes) {
                    ...
                    if ( !defined($WizActive) ) {
                        print
"Hmmm, $Wiz isn't even in the stats list: adding $Wiz now...\n";
                        &SetStat( 'WizActive', $Wiz, time() );
                        next WIZ_CHECK;
                    }

                    $TotScore =
                      $Wizard{$Wiz}{'DuelScore'} +
                      $Wizard{$Wiz}{'MeleeScore'} + 1;

                    $LapsedDays =
                      int( ( time() - $WizActive ) / ( 60 * 60 * 24 ) );

                    if ( $Wizard{$Wiz}{'Retired'} ) {
                        if ( $LapsedDays >
                            $TotScore * $AllowedRetirement * 7 * 4 )
                        {
                            if ( $TotScore > $RetireThreshold ) {
                                print
                                  "$Wiz is enjoying his status in the sun\n";
                            }
                            else {
...
# print "*** $Wiz has had his time in the sun ($LapsedDays, " . ($TotScore-1) . ") - resisiting the temptation... ***\n";
                                ...
                                print
"$Wiz has had his time in the sun ($LapsedDays, $TotScore) - terminating...\n";
                                &OpenMail( LAZY_GUY,
                                    $Users{ $Wizard{$Wiz}{'User'} }
                                      {'Address'} );
                                print LAZY_GUY
"Subject: Remember $Wiz on Firetop Mountain?\n\n";
                        ...
                        print "$Wiz is getting pretty lazy ($LapsedDays, "
                          . ( $TotScore - 1 )
                          . ") - retiring...\n";

                        &OpenMail( LAZY_GUY,
                            $Users{ $Wizard{$Wiz}{'User'} }{'Address'} );
                        print LAZY_GUY
                          "Subject: Remember $Wiz on Firetop Mountain?\n\n";
                        ...
                        print
"$Wiz has been hanging around for $LapsedDays days (OK, score is "
                          . ( $TotScore - 1 ) . ")\n";
                    }
                }
            ...
            if ( !defined($UserActive) ) {
                print
                  "Hmmmm ... $User is not even in the stats.  Adding now...\n";
                &SetStat( 'UserActive', $User, time() );
            }
            else {
                if ( $UserLapsedDays > $TerminateUser ) {
                    print
"$User is stale ($UserLapsedDays days) - terminating...\n";
                    &OpenMail( LAZY_GUY, $Users{$User}{'Address'} );
                    print LAZY_GUY
                      "Subject: Your User $User on Firetop Mountain\n\n";
                    ...
                    print
"$User is hanging around ($UserLapsedDays days): nagging...\n";
                    &OpenMail( LAZY_GUY, $Users{$User}{'Address'} );
                    print LAZY_GUY
                      "Subject: Your User $User on Firetop Mountain\n\n";
                    ...
                    print
                      "$User better get mages soon ($UserLapsedDays days)...\n";
                }
            }
        }
    }
    ...
    open( VACATIONERS, ">$VacationFile" )
      or die "Gleep: couldn't write vacation file!\n";
    ...
    foreach $ChallengeFile ( grep( m/^\d+.ngm$/, @Here ) ) {
        ...
        if ( -M $ChallengeFile > $ChallengeTimeout ) {
            ...
            foreach $Acceptor ( keys(%Accepted) ) {
                ...
                &OpenMail( LAZY_GUY,
                    $Users{ $Wizard{$Acceptor}{'User'} }{'Address'} );
                print LAZY_GUY
"Subject: Remember Challenge $ChallengeNum on Firetop Mountain?\n\n";
                print LAZY_GUY &ChallengeTermination( $Acceptor,
                    $ChallengeNum );
                close LAZY_GUY;
            ...
            unlink($ChallengeFile)
              or warn "Gah! Could not remove $ChallengeFile!\n";
        }
            ...
            &OpenMail( LAZY_GUY,
                $Users{ $Wizard{$Challenger}{'User'} }{'Address'} );
            print LAZY_GUY
"Subject: Remember Challenge $ChallengeNum on Firetop Mountain?\n\n";
