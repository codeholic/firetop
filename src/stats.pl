...
sub SetStat {
    ...
    if ( !$Stats{$Group} ) {
        print STDERR
          "Attempt to write to non-existent stats group $Group: ignoring\n";
        return;
    }
    ...
    if ( $stat =~ m/\s+/ ) {
        print STDERR
          "Attempt to write to stat with space in the name: $stat - ignoring\n";
        return;
    }
...
sub GetStat {
    ...
    if ( !$Stats{$Group} ) {
        print STDERR
          "Attempt to read to non-existent stats group $Group: ignoring\n";
        return;
    }
...
sub IncrementStat {
    ...
    if ( !$Stats{$Group} ) {
        print STDERR
          "Attempt to increment in non-existent stats group $Group: ignoring\n";
        return;
    }
    ...
    $Stats{$Group}{$stat} =
      $Stats{$Group}{$stat} ? $Stats{$Group}{$stat} + $inc_by : $inc_by;
}
...
sub StatsListing {
    ...
    foreach $group ( sort @RequiredStats ) {
        ...
        $Listing .=
          '-' x
          ( length($group) + 2 + length( $Stats{$group}{'Group Description'} ) )
          . "\n";

        foreach $stat (
            sort { SortStatValues( $Stats{$group} ) }
            grep { $_ !~ / / }    # don't include control fields
            keys( %{ $Stats{$group} } )
          )
        {
            $Listing .= sprintf( "%-20s %s\n",
                $stat . ":",
                &{ $Stats{$group}{'Display Func Name'} }(
                    $Stats{$group}{$stat} ) );
        }
    }
    $Listing .= "\n";
...
sub WriteStats {
    ...
    close(STATS)
      or print STDERR "error writing STATS file (bad luck!)\n";
}
