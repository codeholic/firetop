...
%Damage = (
    'Stab',          '1', 'Goblin', '1', 'Ogre',         '2',
    'Troll',         '3', 'Giant',  '4', 'IceElemental', '3',
    'FireElemental', '3'
);
...
@Colours =
  ( 'Red', 'Green', 'Pink', 'Blue', 'Orange', 'Mauve', 'White', 'Yellow' );
...
%RevHandMap = (
    'RH', 'the right hand', 'LH',   'the left hand',
    'BH', 'both hands',     'FIRE', 'a banked spell'
);
...
sub WriteWizards {
    ...
    open( WIZARDS, ">wizards.dat" )
      or die "Couldn't write wizards.dat: $!, stopped";

    print WIZARDS Data::Dumper->Dump( [ \%Wizard ], ['*Wizard'] );
...
sub WriteGameInfo {
    ...
    print GAME Data::Dumper->Dump(
        [
            $Turn,                 $SequenceNumber,
            $MonstersCreated,      $ColourSeed,
            $GesturesDone,         $IceElementalPresent,
            $FireElementalPresent, $SpellBook,
            \@Players,             \@Subscribers,
            \@DeadPlayers,         \@DeadMonsters,
            \@Beings,              \%Info
        ],
        [
            'Turn',                 'SequenceNumber',
...
%TargetSelf = (
    "Dispel Magic",          '1', "Magic Mirror",         '1',
    "Lightning Bolt",        '0', "Cure Heavy Wounds",    '1',
    "Cure Light Wounds",     '1', "Delay Effect",         '1',
    "Raise Dead",            '1', "Shield",               '1',
    "Invisibility",          '1', "Haste",                '1',
    "Permanency",            '1', "Time Stop",            '1',
    "Resist Cold",           '1', "Counter Spell",        '1',
    "Resist Heat",           '1', "Protection",           '1',
    "Summon Goblin",         '1', "Summon Ogre",          '1',
    "Summon Troll",          '1', "Summon Giant",         '1',
    "Summon Fire Elemental", '0', "Summon Ice Elemental", '0',
    "Paralysis",             '0', "Amnesia",              '0',
    "Fear",                  '0', "Confusion",            '0',
    "Charm Monster",         '1', "Charm Person",         '0',
    "Disease",               '0', "Poison",               '0',
    "Blindness",             '0', "Remove Enchantment",   '0',
    "LightningBolt",         '0', "Fireball",             '0',
    "Finger of Death",       '0', "Magic Missile",        '0',
    "Cause Light Wounds",    '0', "Cause Heavy Wounds",   '0',
    "Anti Spell",            '0', "Fire Storm",           '0',
    "Ice Storm",             '0'
);
...
%UntargettedSpell = (
    "Summon Ice Elemental", '1', "Summon Fire Elemental", '1',
    "Fire Storm",           '1', "Ice Storm",             '1'
);
...
sub GameStatus {
    ...
    if ( !$FinishGame ) {
        if (@Receivers) {
            $Status =
                "\u$SpellBook $GameType Status for "
              . &JoinList(@Receivers)
              . " (game $GameName, turn $Turn):\n";
            ...
            for (
                $i = 0 ;
                $i < length( $Info{ $Receivers[0] }{'BlindTurns'} ) ;
                $i++
              )
            {
                $Blind = 1;
            ...
            $Status =
              "\u$SpellBook $GameType Status (game $GameName, turn $Turn):\n";
            for (
                $i = 0 ;
                $i < length( $Info{ $Players[0] }{'BlindTurns'} ) ;
                $i++
              )
            {
                $Blind = 0;
      ...
      )
    {
        ...
        if ( &IsMonster($being) ) {
            ...
            $extra = "";
            foreach $atr (
                'ColdResistant', 'Confused',  'Countering', 'Fast',
                'HeatResistant', 'Invisible', 'Paralyzed',  'Poisoned',
                'Reflecting',    'Shielded',  'Sick'
              )
            {
                if ( $Info{$being}{$atr} ) {
            ...
            if ($NoSee) {
                $Status .=
                  "(You wish you could see what shape $being is in.)\n";
            }
            else {
                $extra = "";
                ...
                foreach $atr (
                    'Afraid',            'Blind',
                    'Charmed',           'ColdResistant',
                    'Confused',          'Countering',
                    'DelayPending',      'Fast',
                    'Forgetful',         'HeatResistant',
                    'Invisible',         'Paralyzed',
                    'PermanencyPending', 'Poisoned',
                    'Reflecting',        'Shielded',
                    'Sick',              'TimeStoppedTurn'
                  )
                {
                    if ( $Info{$being}{$atr} ) {
                        $Status .= "$atr($Info{$being}{$atr}) ";
            ...
            if (   $Info{$being}{'State'} eq 'orders'
                or $Info{$being}{'State'} eq 'Dead'
                or $Info{$being}{'State'} eq 'OUT'
                or $FinishGame )
            {
                $Amount = length( $Info{$being}{'RH'} );
            }
            else {    # They must have submitted gestures for this turn
...
sub LastGesture {
    ...
    my ( $o, $c );
    $o = 1;
    $o += 1 while ( $c = substr( $String, -$o, 1 ) ) eq '.' or $c eq '#';
    return $c;
}

@NoEVerbs =
  ( 'wish', 'do', 'flinch' );    # eg flinches -> flinch .. no 2nd person "e".
...
sub SecondPerson {
    ...
    $String =~
      s/ at RH:(.+)\b/, anticipating a monster from $1\'s right spell/g;
...
sub RemoveAllEnchantments {
    ...
    (
        $Info{$Name}{'StruckBlind'},       $Info{$Name}{'Disappearing'},
        $Info{$Name}{'Forgetful'},         $Info{$Name}{'Confused'},
        $Info{$Name}{'Charmed'},           $Info{$Name}{'CharmedLeft'},
        $Info{$Name}{'CharmedRight'},      $Info{$Name}{'Paralyzed'},
        $Info{$Name}{'ParalyzedLeft'},     $Info{$Name}{'ParalyzedRight'},
        $Info{$Name}{'Afraid'},            $Info{$Name}{'ColdResistant'},
        $Info{$Name}{'HeatResistant'},     $Info{$Name}{'Sick'},
        $Info{$Name}{'Poisoned'},          $Info{$Name}{'Blind'},
        $Info{$Name}{'Invisible'},         $Info{$Name}{'Fast'},
        $Info{$Name}{'Lost'},              $Info{$Name}{'Shielded'},
        $Info{$Name}{'PermanencyPending'}, $Info{$Name}{'DelayPending'},
        $Info{$Name}{'TimeStopped'},       $Info{$Name}{'SavedSpell'},
...
sub GameList {
    local (@Gamelist) = @_;
    my (
        $Wiz,  $state, $Games,     $NextGame, $GameTurn,
        $desc, $line,  @GameFiles, @DscFiles
    ) = ( "", "", "", 0, 0 );
    ...
    if ( !@Gamelist ) {
        @GameFiles =
          sort { &Num($a) <=> &Num($b) } grep( /\.gm$/, readdir(GAMEDIR) );
    ...
    if (@ShowGames) {
        ...
        $Games .=
" Game turn              Wizard  State                  Wizard  State\n";
        $Games .=
" -------------------------------------------------------------------\n\n";
    }
        ...
        while ( $Wizard = shift(@Beings) ) {
                ...
                && ( $Users{ $Wizard{$Wizard}{'User'} }{'Vacation'} ) )
            {
                ...
                if (    $StateDescription{$State}
                    and $StateDescription{$State} eq 'thinking' )
                {
                    $State = "";
                }
                else {
    ...
    if ( !@Gamelist ) {
        rewinddir(GAMEDIR);
        @DscFiles =
          sort { &Num($a) <=> &Num($b) } grep( /.dsc$/, readdir(GAMEDIR) );
    }
    else {
...
sub wizard_by_score {
    if ( $Wizard{$a}{ $ScoreType . 'Score' } !=
        $Wizard{$b}{ $ScoreType . 'Score' } )
    {
        return $Wizard{$b}{ $ScoreType . 'Score' } <=> $Wizard{$a}
          { $ScoreType . 'Score' };
    }
    ...
    foreach $wiz (@Wizzes) {
      ...
      # If two mages are blind at the same time, they should not see each other.
        ...
        if ( length( $Hist{$wiz}{'Monsters'} ) > 13 ) {
            ...
            $Hist{$wiz}{'ExMonsters'} =
              substr( $Hist{$wiz}{'Monsters'}, $i + 1 );
            $Hist{$wiz}{'Monsters'} = substr( $Hist{$wiz}{'Monsters'}, 0, $i );
        }

        printf( HISTORY "%s %-2s %-2s %2d %-7s %-13s :",
            $Hist{$wiz}{'Hide'},   $Hist{$wiz}{'LH'},
            $Hist{$wiz}{'RH'},     $Info{$wiz}{'HP'},
            $Hist{$wiz}{'Spells'}, $Hist{$wiz}{'Monsters'}
        );
    }
...
sub EndDescription {
    ...
    open( DESC, ">$GameName.dsc.new" )
      || die "Cannot write to $GameName.dsc.new\n$!";
    $line = <OLD>;    # Skip title
    print DESC $line;
    $line = <OLD>;    # Skip ----- line
    print DESC $line;
    ...
    close DESC;
    close OLD;
    rename( "$GameName.dsc", "$GameName.dsc.old" )
      || die "Error renaming $GameName.dsc\n$!";
    rename( "$GameName.dsc.new", "$GameName.dsc" )
      || die "Error renaming $GameName.dsc.new\n$!";
...
sub ScoreGame {
    ...
    foreach $wiz (@Players) {
        ...
        if ( $Info{$wiz}{'HP'} <= 0 ) {
            ...
            print HISTORY
              "$wiz $Wizard{$wiz}{$GameType.'Score'} $Wizard{$wiz}{'User'}\n";
            close HISTORY;
    ...
    if ( $#sameplr == $#Players ) {
        ...
        if ( $#Players > 1 && $#Winners > -1 ) {
            ...
            &Announce(
"The reward for winning the $GameType is $GameValue point$plural.\n"
            );
        }
        if ( $#Winners > 0 ) {
            $num       = $#Winners + 1;
            ...
            $plural    = ( $GameValue == 1 ) ? "" : "s";
            &Announce(
"The reward is divided amongst the $num winners (rounding down).\n"
...
sub OpenMail {
    ...
    if ( !$Addr ) { # looks like the mage disappeared! No drama - it can happen.
        print LOG
          "OpenMail opening /dev/null 'cause it wasn't passed an address...\n";
    ...
    open( $FileHandle, "|tee -a $LogName|$sendmail -f$gmAddr $Addr" )
      || die "Could not open sendmail to $Addr\n$!\n";
...
sub SendEsquireMail {
    ...
    open( ESQUIRE, "|$sendmail esquire\@gamerz.net" )
      || die "Could not open sendmail to esquire\@gamerz.net\n$!\n";
    select( ( select(ESQUIRE), $| = 1 )[0] );
    print ESQUIRE
"To: esquire\@gamerz.net\nSender: fm-users-owner\@gamerz.net\nReply-To: nobody\@gamerz.net\nSubject: Commands to esquire\n\n";
...
sub OnVacation {
    ...
    if ( !$Referee and $Duration > 7 * $AllowedVacation ) {
        print MAIL
" Sorry - we're all too jealous to allow vacations longer than $AllowedVacation weeks!\n";
        return;
    }
        ...
        if ( $VacUser eq $Active ) {
            ...
            foreach $Wiz ( &WizardsOfUser( $Active, @WizardNames ) ) {
                ...
                if ( $game and $game !~ s/^N// ) {
                    ...
                    if ( $StateDescription{$state} ) {
                        print MAIL
" Your wizard $Wiz is awaiting your orders in game $Wizard{$Wiz}{'Busy'}.\n"
