...
sub DeclineAcknowledge {
    local ($Wizard) = @_;
    return
"Your opponent(s) graciously acknowlenge your decision not to face a $GameType.
...
sub DisplayNewGameState {
    ...
    if ( @Wizlist == 1 ) {
        $disp .=
            " $Challenger is patiently sitting alone in the Graven Circle,\n"
    ...
    if ($Open) {
        ...
        if ( $Open =~ /^\d+$/ ) {
            my ($s) = ( $Open > 1 ) ? "s" : "";
            $disp .=
" (But each player may only accept this challenge with $Open mage$s.)\n";
        }
    }
    else {
        if ( !$Challenged[0] ) {
...
"\n $Challenged[0] is challenged to participate in this $GameType,\n"
    ...
    $disp .=
      "\n This battle is limited to a total of " . "$Limit participants.\n"
      if $Limit;
    ...
    if ($Auth) {
        ...
        if (@Changes) {
            ...
            foreach $Change (@Changes) {
                ...
                $disp .=
                  "  These mages are for change $Change: "
                  . &JoinList(@For) . "\n"
                  if @For;
                $disp .=
                  "  These mages are against change $Change: "
                  . &JoinList(@Against) . "\n"
                  if @Against;
                $disp .= "\n\n";
            }
...
sub DisplayChange {
    ...
    if ($NewOpen) {
        if ( $NewOpen eq "No" ) {
            $disp .=
"   Close the challenge, so that only those that are challenged can join.\n";
        }
        else {
...
",\n   but each player may control maximum $NewOpen mage$s.\n";
    ...
    if ($StartNow) {
        $disp .=
"   Start the game now, and exclude those that have not accepted to join yet.\n";
    }
