...
if ( $ENV{'FM_SITE'} and ( "pvv" eq $ENV{FM_SITE} ) ) {
    ...
    $InputFile           = shift
      or die
      "Need to supply input file name as second argument to process.pl\n";
    ...
    $RequiredSubject = "FM Orders";
    $UpdateFMUsers =
      1;    # If addresses should be sendt to the FM-Users mailing list
    ...
    @MaintainerAddresses = (
        "craigferguson\@shaw.ca", "yes\@barak.net.il",
        "andy\@toucanet.com",     "mgregory\@ieee.org",
        "neonelf\@yahoo.com",     "akur\@gamerz.net",
        "janitor\@gregories.net", "wsimmons\@unicircuit.com",
        "martin\@gregories.net",  "yuvalsapir\@gmail.com"
    );
    $InputFile      = $ARGV[0];
    $CallProcess_pl = "|$Home/process.pl $Home -";
