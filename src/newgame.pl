...
@OptionsList = (
    'add',     'challenge', 'exclude',       'start',
    'all',     'open',      'closed',        'limit',
    'nolimit', 'spellbook', 'authoritarian', 'democratic'
);

sub ReadNewGame {
    my ($NewGame) = @_;

    open( NEWGAME, "$NewGame.ngm" )
      || die "Could not open file $NewGame.ngm\n$!";
...
sub WriteNewGame {
    ...
    if ( $Limit == 2 ) {
        ...
        if ( !$Open ) {
            $GameType =
              ( scalar( keys(%Accepted) ) + @Challenged > 2 )
              ? "Melee"
              : "Duel";
        }
        else {
    ...
    open( NEWGAME, ">$NewGame.ngm" )
      or die "Can't write $NewGame.ngm: $!, stopped ";

    print LOG "Writing setup $NewGame\n";

    print NEWGAME Data::Dumper->Dump(
        [
            $Comment,   $Open,        $Limit,     $Auth,
            $DemTime,   $GameType,    $SpellBook, $Challenger,
            \%Accepted, \@Challenged, \@Excluded, \@Subscribers
        ],
        [
            'Comment',   'Open',        'Limit',     'Auth',
            'DemTime',   'GameType',    'SpellBook', 'Challenger',
    ...
    close(NEWGAME)
      or die "Couldn't write $NewGame.ngm: $!, stopped ";
}
...
sub ReadChange {
    ...
    while ( ( $_ = <CHANGE> ) ne "---BEGIN CHALLENGED---\n" ) {
        ...
        next if ( !m/\S+/ );
        ( $Field, $Value ) = m/^(\w+)\:(.*)$/
          or die "Error in file $NewGame-C-$Change.ngm\n";
        ...
        while ( $#arg >= 0 ) {
            ...
            if ( $curroption eq "spellbook" ) {
                    ...
                    and grep { $arg[0] =~ /^$_/i } @choises )
                {
                    ...
                    $msg .=
"> The option 'spellbook' must have one of the following arguments:\n";
                    my ($book);
             ...
             #}
                ...
                if ( $#arg < 0 || $arg[0] !~ /^[\d]+$/ ) {
                    ...
                    $msg .=
"> You cannot limit the challenge to less than 2 players\n";
                    $errs++;
                }
                $Limit = shift(@arg);
            }
            ...
            #      }
                ...
                my @Mages = ();
                while ( defined( $arg[0] ) && !grep { $arg[0] eq $_ }
                    @OptionsList )
                {
                ...
                if ( $#Mages < 0 ) {
                    $msg .=
"> $curroption does not make much sense without at least one wizard!\n";
                    $errs++;
                }
                ...
                foreach $Mage (@Mages) {
                    ...
                    if ( $Mage eq $Challenger ) {
                        $msg .=
"You want to challenge your self? Are you shizofren?\n";
                        $errs++;
                    }
                    ...
                    if ( $Wizard{$Mage}{'Busy'} ) {
                        ...
                        if ( $Game !~ s/^N// ) {
                            $msg .=
"> $Mage is already engaged in battle $Game at this time!\n";
                        }
                        else {
                            $msg .=
                              "> $Mage has already accepted challenge $Game\n";
                        }
                        $errs++;
                    }
...
"> $curroption does not make much sense without at least one wizard!\n";
                ...
                foreach $Mage (@Mages) {
                    ...
                    if ( !$found and !$Open ) {
                        $msg .=
"> $Mage has not been challenged in the first place.\n";
                        $errs++;
                    }
              ...
              MAGE: foreach $Mage (@WizardNames) {
                    next MAGE
                      if $Mage eq $Challenger
                          or $Wizard{$Mage}{'Busy'}
                          or $Wizard{$Mage}{'Retired'}
                          or grep ( /^$Mage$/, @Excluded );
...
sub ChangeNewGame {
    ...
    if ( !$Accepted{$Proposer} ) {
        print MAIL
          "$h You are not among those that have accepted challenge $NewGame.\n";
    ...
    if ($Auth) {
        if ( $Proposer eq $Challenger ) {
            ...
            $msg =
              "$h Only $Challenger can do changes to challenge $NewGame.\n\n";
            print MAIL $msg;
            return;
        }
    }
...
"> The option 'spellbook' MUST have one of the following arguments:\n";
             ...
             #}
                ...
                if ( defined( $arg[0] ) && ( $arg[0] =~ /^[\d]+$/ ) ) {
                    ...
                    if ( $limitparm < 2 ) {
                        $msg .=
"> You cannot limit the game to less than 2 players\n";
                        $errs++;
                    }
                    elsif ( $limitparm < $numAccepted ) {
                        $msg .=
"> $numAccepted mages have already joined this new game,\n"
                    ...
                    $msg .=
                      "> The option 'limit' MUST have one numeric argument.\n";
                    $errs++;
                }
            }
...
"> You want to challenge your self? Are you shizofren?\n";
                    ...
                    if ( $Accepted{$Mage} ) {
                        $msg .=
"> $Mage has allready accpted to take part in this battle.\n";
                        $errs++;
                    }
...
"> $Mage has not been challenged in the first place.\n";
                ...
                foreach $Mage (@WizardNames) {
                    if (
                        !(
                               $Accepted{$Mage}
                            or grep ( /^$Mage$/, ( @Challenged, @Excluded ) )
                            or $Wizard{$Mage}{'Busy'}
                            or $Wizard{$Mage}{'Retired'}
                        )
                      )
    ...
    if ( $numAccepted == 2 ) {
        $msg =
"50% of those that have accepted to join this game are for your proposed change.\n";
        if ( $Proposer eq $Challenger ) {
            $msg .=
              "You initiated this challenge and therfore have a double vote,\n"
    ...
    foreach $Name ( keys %Accepted ) {
        ...
        print CHANGE &ChangeProposalNotification( $Proposer, $Name, $NewGame,
            $Change );
...
sub DoChange {
    ...
    if ( !$StartNow and @NewChallenged ) {
        for ( $m = 0 ; $m <= $#NewChallenged ; $m++ ) {
          EXCL: foreach $n ( 0 .. $#Excluded ) {
            ...
            if (   $Wizard{ $NewChallenged[$m] }{'Busy'}
                or $Wizard{ $NewChallenged[$m] }{'Retired'} )
            {
            ...
            foreach $Mage (@NewExcluded) {
              CHALL: for ( $n = 0 ; $n <= $#Challenged ; $n++ ) {
                    if ( $Challenged[$n] eq $Mage ) {
                        splice( @Challenged, $n, 1 );
                        &OpenMail( EXCLUDED,
                            $Users{ $Wizard{$Mage}{'User'} }{'Address'} );
    ...
    if (   ( $StartNow and $numAccepted > 1 )
        or ( !$Open and !@Challenged and $numAccepted > 1 )
        or ( $numAccepted == $Limit ) )
    {
        &StartGame;
        return;
    }
    ...
    if ( !$Accepted{$Player} ) {
        if ($Open) {
            if ( grep ( /^$Player$/, @Excluded ) ) {
                print MAIL
"$h Sorry, but you are excluded from joining this battle.\n\n";
                print MAIL $GameState;
                return;
            }
            ...
            if ( $Open =~ m/^\d+$/ ) {
                ...
                if ( $num >= $Open ) {
                    $s = 's' if $Open > 1;
                    print MAIL
"$h Only $Open mage$s pr. player is allowed in this game.\n";
                    print MAIL
                      " > You have already joined this game with the mage$s "
                      . &JoinList(@Same) . ".\n\n";
                    print MAIL $GameState;
                    return;
                }
            ...
            if ( !$Accepted{$Player} ) {
                print MAIL
                  "$h Sorry, but you are not challenged to join this game.\n\n";
                print MAIL $GameState;
                return;
            }
        }
        ...
        local ($AccNote);
        $AccNote =
          &AcceptNotification( $Player, $Wizard{$Player}{'User'}, $NewGame );
        ...
        print MAIL
"Since the participants have not all arrived, you sit down to wait.\n\n";
    }
    ...
    if ( $key eq "everything" ) {
        print MAIL
"You tell the referee that you accept any changes anyone may propose.\n\n";
        ...
        print MAIL
"You tell the referee that you accept the proposed change no $Change.\n\n";
    ...
    foreach ( readdir(DIR) ) {
        if (/^(\d+)\.ngm$/) {
            ...
            if ( @Challenged || $Open ) {
                foreach $Name ( keys %Accepted, @Challenged ) {
                    &OpenMail( OPPONENT,
                        $Users{ $Wizard{$Name}{'User'} }{'Address'} );
...
sub CheckChange {
    ...
    if ( $For > $numAccepted ) {
        ...
        foreach $Name ( keys %Accepted ) {
            next if $Name eq $Player;
            &OpenMail( OPPONENT, $Users{ $Wizard{$Name}{'User'} }{'Address'} );
            print OPPONENT &NoChangeNotification( $Proposer, $Name, $NewGame,
                $Change );
            print OPPONENT $GameState;
            close OPPONENT;
        }
        print MAIL
"The majority of the wizards that have agreed to join the new game $NewGame,
...
sub Oppose {
    ...
    print MAIL
"You tell the referee that you oppose the proposed change no $Change.\n\n";
...
sub Decline {
    ...
    if ( $Accepted{$Player} ) {
        ...
        if ( !$found ) {
            print MAIL
"$h It doesn't seem like you are challenged to join this new game\n";
            print MAIL
              "   So luckily no-one knows you would have wimped out!\n\n";
            print MAIL $GameState;
            return;
        }
    }
...
sub Withdraw {
    ...
    foreach $Name ( keys %Accepted, @Challenged ) {
        ...
        close OPPONENT;
        print MAIL
" $Name has been notified of your decision not to proceed with new game $NewGame.\n\n";
    }
...
sub NewGameList {
    my ( $NewGame, $Line, $List, $NumAcc, $NumChl, @OpenChallenges,
        @ClosedCallenges );
    ...
    foreach ( readdir(SAVEDIR) ) {
        if (/^([^-]+)\.ngm$/) {
            $NewGame = $1;
            &ReadNewGame($NewGame)
              || die "NewGameList could not open game $NewGame\n$!\n";
            if ($Open) {
                $NumAcc = ( keys %Accepted ) - 1;

                $Line = sprintf(
                    "   %4d   %-15s %-14s %5d     %5s  %s",
                    $NewGame, $Challenger, "($Wizard{$Challenger}{'User'})",
                    $NumAcc, ( $Limit ? $Limit : 'none' ),
                    "\u$SpellBook"
                );
                ...
                $Line   = sprintf(
                    "   %4d   %-15s %-14s %5d     %2d (%4s)",
                    $NewGame, $Challenger, "($Wizard{$Challenger}{'User'})",
                    $NumAcc, $NumChl, ( $Limit ? $Limit : 'none' )
                );
                $Line .= "    Comment: $Comment" if $Comment;
                $Line .= "\n";
    ...
    if (@OpenChallenges) {
        ...
        $List .=
"   Game   Challenger                    No. acc.   Limit  Spell Book\n";
        $List .= "   " . '-' x 64 . "\n";
