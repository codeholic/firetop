...
sub DispelMagic {
    ...
    if ( $Action eq 'Reflecting' ) {
        $Eff =
          "As the shock wave hits the shimmering glow surrounding $Target,\n";
...
sub CounterSpell {
    ...
    local (%Strings) = (
        NoTarget =>
"$Caster\'s Counterspell zaps across the circle at a non-existent target!\n",
        Blind =>
          "$Caster fires a Counterspell, trying to guess where $Target is...\n"
          . "$Caster\'s Counterspell misses $Target.\n",
        DeadTarget => "$Caster\'s Counterspell is wasted.\n",
        Invisible =>
"$Caster\'s Counterspell completely misses its invisible target ($Target)!\n",
        Countering =>
"$Caster\'s Counterspell joins the other, forming a hazy glow around $Target.\n",
...
sub MagicMirror {
    ...
    local (%Strings) = (
        NoTarget =>
"$Caster\'s Magic Mirror zaps across the circle at a non-existent target!\n",
        Blind =>
          "$Caster fires a Magic Mirror, trying to guess where $Target is...\n"
          . "$Caster\'s Magic Mirror misses $Target.\n",
        DeadTarget => "$Caster\'s Magic Mirror is wasted.\n",
        Invisible =>
"$Caster\'s Magic Mirror completely misses its invisible target ($Target)!\n",
        Countering =>
          "The hazy glow surrounding $Target shimmers for a moment.\n",
...
sub Shield {
    ...
    local (%Strings) = (
        NoTarget =>
"$Caster\'s Shield zaps across the circle at a non-existent target!\n",
        ...
        Invisible =>
"$Caster\'s Shield completely misses its invisible target ($Target)!\n",
        Countering =>
          "A glimmering shield dissolves in $Target\'s hazy glow.\n",
...
sub Protection {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( !$Info{$Target}{'Invisible'} ) {
            $String .=
              "A solid, glimmering shield appears in front of $Target.\n";
        }
        else {
        ...
        if ( $Info{$Target}{'Shielded'} >= 0 ) {
            $Info{$Target}{'Shielded'} = ( $duration == 999 ? 999 : 4 );
        }
    ...
    local (%Strings) = (
        NoTarget =>
"$Caster\'s Protection zaps across the circle at a non-existent target!\n",
        Blind =>
          "$Caster casts Protection, trying to guess where $Target is...\n"
          . "$Caster\'s Protection misses $Target.\n",
        DeadTarget => "$Caster\'s Protection is wasted.\n",
        Invisible =>
"$Caster\'s Protection completely misses its invisible target ($Target)!\n",
        Countering =>
          "A solid glimmering shield is engulfed in $Target\'s hazy glow.\n",
...
sub ResistHeat {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( $Target eq 'FireElemental' ) {
            ...
            $String .=
              "the Fire Elemental withers, then, with a 'pop', disappears!\n";
        }
    ...
    local (%Strings) = (
        NoTarget =>
"$Caster\'s Resist Heat zaps across the circle at a non-existent target!\n",
        Blind =>
          "$Caster casts Resist Heat, trying to guess where $Target is...\n"
          . "$Caster\'s Resist Heat misses $Target.\n",
        DeadTarget => "$Caster\'s Resist Heat is wasted.\n",
        Invisible =>
"$Caster\'s Resist Heat completely misses its invisible target ($Target)!\n",
        Countering => "Powdery frost sprinkes off $Target\'s hazy glow.\n",
        Reflecting =>
"$Caster\'s Resist Heat spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub ResistCold {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( $Target eq 'IceElemental' ) {
            $String .= "As totaly contrary magic tries to take hold of it,\n";
            $String .=
"the Ice Elemental withers, then, with a sharp 'crack', disappears!\n";
        }
        elsif ( !$Info{$Target}{'Invisible'} ) {
    ...
    local (%Strings) = (
        NoTarget =>
"$Caster\'s Resist Cold zaps across the circle at a non-existent target!\n",
        Blind =>
          "$Caster casts Resist Cold, trying to guess where $Target is...\n"
          . "$Caster\'s Resist Cold misses $Target.\n",
        DeadTarget => "$Caster\'s Resist Cold is wasted.\n",
        Invisible =>
"$Caster\'s Resist Cold completely misses its invisible target ($Target)!\n",
        Countering => "$Target\'s hazy glow turns deep red for a moment.\n",
        Reflecting =>
"$Caster\'s Resist Cold spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub SummonMonster {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( $Attack eq 'no_one' ) {
            return
"-T $NewName springs into being and looks at $Info{$NewName}{'Controller'} for instructions.\n";
        }
        else {
    ...
    local (%Strings) = (
        NoMonster =>
          "$Caster\'s attempt to cast a Summon spell at a being that\n"
          . "did not exist from the beginning of the turn fails miserably.\n",
        NoTarget =>
"$Caster\'s $Spell zaps across the circle at a non-existent target!\n",
        Blind => "$Caster casts $Spell, trying to guess where $Target is...\n"
          . "$Caster\'s $Spell misses $Target.\n",
        DeadTarget => "$Caster\'s $Spell is wasted.\n",
        Invisible =>
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Countering =>
"-T As a $Monster springs into being, it is absorbed by $Target\'s hazy glow.\n",
        Reflecting =>
"-T $Caster\'s $Spell spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub SummonElemental {
    ...
    my ($Effect) =
"A $ThisElement Elemental springs into being, and $Screams with indignation!\n";

    if ($FireAndIceExplosion) {
        $Effect .=
          "The $ThisElement Elemental dies in the shower of ice and flame.\n";
        ...
        $Effect .=
          "The $OtherElement Elements howls as it sees a worthy foe!\n";
  ...
  #Generic routine to check if an enchantment cancels against an other.
{
    ...
    if (
        $Info{$Target}{'EnchantCancelled'} &&    # might not be initialised
        $Info{$Target}{'EnchantCancelled'} > 1
      )
    {
        return
"Another enchantment whirls around $Target, and crackles away to nothing.\n";
    }
    elsif ( $SpellType eq 'Paralysis' ) {
        if ( $Info{$Target}{'EnchantCancelled'} ) {
            return
"Another paralysis whirls around $Target, and crackles away to nothing.\n";
        }
            ...
            || $Info{$Target}{'EnchantCancelled'} )
        {
            $Info{$Target}{'Paralyzed'} =
              Min( &Num( $Info{$Target}{'Paralyzed'} ), 0 );
            return
"The Paralysis taking hold of $Target gives way to a more potent effect...\n";
            ...
            || ( $Info{$Target}{'Afraid'} > 0 ) )
        {
            ...
            $Info{$Target}{'Confused'} =
              Min( &Num( $Info{$Target}{'Confused'} ), 0 );
            $Info{$Target}{'Charmed'} =
              Min( &Num( $Info{$Target}{'Charmed'} ), 0 );
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( !$Info{$Target}{'EnchantCancelled'} ) {
            ...
            if ( $Info{$Target}{'ParalyzedRight'} ) {
                $String .=
"It looks like $Target\'s right hand is going to be stuck again...\n";
                $Info{$Target}{'ParalyzedRight'} = $Info{$Target}{'Paralyzed'};
            }
            elsif ( $Info{$Target}{'ParalyzedLeft'} ) {
                $String .=
"It looks like $Target\'s left hand is going to be stuck again...\n";
                $Info{$Target}{'ParalyzedLeft'} = $Info{$Target}{'Paralyzed'};
            }
            else {
                $Info{$Target}{'Paralyser'} = $Caster;
                $String .=
                  "$Caster\'s Paralysis spell takes hold of $Target.\n";
            }
        }
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Countering =>
          "$Caster\'s $Spell spell fizzles against $Target\'s hazy glow.\n",
        Reflecting =>
"$Caster\'s $Spell spell reflects off the shimmering glow surrounding $Target!\n"
    );
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( !$Info{$Target}{'EnchantCancelled'} ) {
            if ( $Info{$Target}{'Afraid'} >= 0 ) {
                $String .=
                  "Suddenly, $Target starts to look very frightened!\n";
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( !$Info{$Target}{'EnchantCancelled'} ) {
            if ( $Info{$Target}{'Confused'} >= 0 ) {
                $String .=
"All of a sudden, $Target starts to find things rather confusing...\n";
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( !$Info{$Target}{'EnchantCancelled'} ) {
            if ( &IsMonster($Target) ) {
                ...
                $String .=
"$Target ignores $Caster\'s appeal to $Target\'s baser instincts.\n";
                $Info{$Target}{'EnchantCancelled'} = 2;
            }
        }
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        ...
        Reflecting =>
"The shimmering glow surrounding $Target somehow \nmakes $Target look very charming to $Caster.\n"
    );
...
sub CharmPerson {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( !$Info{$Target}{'EnchantCancelled'} ) {
            if ( &IsMonster($Target) ) {
                $String .=
"$Target doesn't find $Caster\'s human antics particularly charming.\n";
                $Info{$Target}{'EnchantCancelled'} = 2;
            }
            else {
...
sub Haste {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( $Info{$Target}{'Fast'} >= 0 ) {
            ...
            $Info{$Target}{'Fast'} = ( $duration == 999 ? 999 : 3 );
    ...
    local (%Strings) = (
        NoTarget =>
"$Caster\'s $Spell spell zaps across the circle at a non-existent target!\n",
        Blind => "$Caster casts $Spell, trying to guess where $Target is...\n"
...
sub RaiseDead {
    ...
    if ( $Action eq 'DoSpell' )    # must be casting at a live target.
    {
        ...
        $String .=
          "$Target feels streams of life flowing through $Target\'s body.\n";
        ...
        if ( $Info{$Target}{'NewRisen'} ) {
            if ( $Info{$Target}{'Controller'} ne $Caster ) {
                ...
                $String .=
"... $Target does not know to who to be thankful to or whom to obey!\n";
            }
        }
    ...
    if ( $Action eq 'DeadTarget' ) {
        ...
        if ( grep( $Target eq $_, @DeadMonsters ) ) {
            $String .=
"... the decaying remains of the body of the $Target rises up and are healed.\n\n";

            if ( $Target =~ m/((Ice)|(Fire)) *Elemental/i ) {
                my ($ThisElement) = $1;
                my ($OtherElement) =
                  ( $ThisElement eq 'Fire' ) ? "Ice" : "Fire";
                my ($Screams) =
                  ( $ThisElement eq 'Fire' ) ? "roars" : "shrieks";

                $String .= "$Target is alive, and $Screams with indignation!\n";
                if ($FireAndIceExplosion) {
                    $String .=
"The $Target dies again in the shower of ice and flame.\n";
                    return $String;
                }
...
"The $OtherElement Elements howls as it sees a worthy foe!\n";
                    ...
                    $String .=
                      "... and disappear in a shower of ice and flame.\n";
                    &KillBeing("${OtherElement}Elemental");
                    ${"${OtherElement}ElementalPresent"} = 0;
                    $FireAndIceExplosion = 1;
                    return $String;
                ...
                if ( $Attack eq 'no_one' ) {
                    $String .=
"$Target is alive, and looks at $Info{$Target}{'Controller'} for instructions.\n";
                }
                else {
        ...
        if ( grep( $Target eq $_, @DeadPlayers ) ) {
            if ( $Info{$Target}{'State'} eq 'OUT' ) {
                $String .=
"$Target, sitting watching the battle from the side, feels wonderfully alive!\n";
                return $String;
            }
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Countering =>
          "The hazy glow surrounding $Target swirls anti-clockwise.\n",
        Reflecting =>
"$Caster\'s $Spell spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        ...
        Reflecting =>
"A stream of deadly virus reflects off the shimmering glow surrounding $Target!\n"
    );
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        ...
        Reflecting =>
"A stream of deadly venom bounces off the shimmering glow surrounding $Target!\n"
    );
...
sub Blindness {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( &IsMonster($Target) ) {
            ...
            $String .=
              "$Target wanders off aimlessly, then just disintegrates.\n";
            &KillBeing($Target);
        }
        elsif ( $Info{$Target}{'Blind'} >= 0 ) {
            $String = "-T ";
            $String .= "$Target finds the world starting to look dim.\n";
            $Info{$Target}{'StruckBlind'} = ( $duration == 999 ? 999 : 4 );
        }
        else {
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        ...
        Reflecting =>
"A bright flash reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub Invisibility {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        if ( &IsMonster($Target) ) {
            ...
            $Info{$Target}{'Disappearing'} = ( $duration == 999 ? 999 : 4 );
        }
...
sub RemoveEnchantment {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        my ($String) =
          "Tendrils reach out and pick at $Target, absorbing magical energy.\n";
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Countering =>
"Tendrils reaching towards $Target are cut short as they hit a hazy glow.\n",
        Reflecting =>
          "Tendrils reaching towards $Target bounce off a shimmering glow!\n"
    );
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        ...
        Reflecting =>
"$Caster\'s $Spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub FingerofDeath {
    ...
    if ( $Action eq 'Countering' ) {
        ...
        return
"$Caster\'s Death Magic penetrates the hazy glow surrounding $Target!\n$Target feels the touch of the Finger of Death...\n";
    }
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Reflecting =>
"$Caster\'s Death Magic reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub MagicMissile {
    ...
    if ( $Action eq 'Reflecting' ) {
        ...
        $String .=
"The Magic Missile reflects off the shimmering glow surrounding $Target!\n";
        $InitSpell = 1;
        return $String;
    }
    ...
    if ( ( $Action eq 'DoSpell' ) or ( $Action eq 'Countering' ) ) {
        if ($InitSpell) {
            $String =
              "A Magic Missile zaps across the circle back to $Target!\n";
        }
        else {
            $String =
              "A Magic Missile zaps across the circle towards $Target!\n";
        }
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Countering =>
"A jagged hole appears momentarily in the hazy glow surrounding $Target.\n",
        Reflecting =>
"$Caster\'s $Spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Countering =>
          "The hazy glow surrounding $Target thickens and swirls.\n",
        Reflecting =>
"$Caster\'s $Spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub Permanency {
    ...
    if ( $Action eq 'DoSpell' ) {
        ...
        return
"-T $Target prepares to make a spell permanent!\nA blue halo appears above $Target\'s head.\n";
    }
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Countering =>
          "The hazy glow surrounding $Target turns solid for a moment.\n",
        Reflecting =>
"$Caster\'s $Spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub DelayEffect {
    ...
    if ( $Action eq 'DoSpell' ) {
        if (   $Info{$Target}{'DelayPending'} >= 0
            && $Info{$Target}{'DelayPending'} < $duration )
        {
...
"$Caster\'s $Spell completely misses its invisible target ($Target)!\n",
        Countering =>
"The hazy glow surrounding $Target swallows the Delay Effect spell.\n",
        Reflecting =>
"$Caster\'s $Spell reflects off the shimmering glow surrounding $Target!\n"
    );
...
sub FireStorm {
    ...
    if ($FireAndIceStormExplosion) {
        ...
        $Effect .=
          "The Fire Storm is consumed in the shower of fire and ice.\n";
    }
    elsif ($IceElementalPresent) {
        $Effect .= "The Ice Elemental wails as it is consumed in the flames!\n";
        $Effect .=
"With a tremendous hissing of steam and cracking of ice, the Fire Storm abates.\n";
    ...
    if ($FireElementalPresent) {
        $Effect .=
"The FireElemental roars with delight as it surrenders itself to the storm!\n";
...
sub IceStorm {
    ...
    if ($FireAndIceStormExplosion) {
        ...
        $Effect .=
          "The Fire Elemental howls as it is consumed by the icy wind!\n";
        $Effect .=
"With a tremendous hissing of steam and cracking of ice, the Ice Storm abates.\n";
    ...
    if ( $IceElementalPresent && $IceStormActive ) {
        $Effect .=
"The IceElemental wails with delight as it surrenders itself to the storm!\n";
