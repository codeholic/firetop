...
sub CreateUser {
    ...
    if ( &IsMonster($NewUser) ) {
        ...
        print MAIL
          "A Monster posing as a user was turned away from Firetop Mountain!\n";
        return 0;
    }
...
sub WriteUsers {
    ...
    open( USERS, ">users.dat" )
      or die "Error writing users.dat:$!, stopped ";

    print USERS Data::Dumper->Dump( [ \%Users ], ['*Users'] );

    close USERS
      or die "Couldn't write users.dat: $!, stopped ";
}

sub UserList {
    local (@list) = @_;
    my ( $UsrName, @Wizlist, $wiz, $DuelScore, $MeleeScore, $line,
        $OnVacation );
    ...
    foreach $UsrName ( @list ? @list : sort(@UserNames) ) {
        ...
        $line = sprintf(
            "%-20s %4d/%-4d %4d/%-4d %3d/%-3d %s",
            $UsrName . $OnVacation,     $DuelScore,
            $Users{$UsrName}{'Duels'},  $MeleeScore,
            $Users{$UsrName}{'Melees'}, $#Wizlist + 1,
            $Users{$UsrName}{'Mages'},  $Users{$UsrName}{'Address'}
        );
        print MAIL "$line\n";
    }
    print MAIL "\n";
...
sub UserInfoString {
    ...
    foreach my $wiz (@Wizlist) {
        ...
        $MageList .=
          " $wiz ($Wizard{$wiz}{'DuelScore'} $Wizard{$wiz}{'MeleeScore'}";
...
sub WizInfoString {
    ...
    my ($String) =
      (     "Mage name:        " 
          . $Name . "\n"
          . "Duel score:       "
          . $Wizard{$Name}{'DuelScore'} . "\n"
          . "Melee score:      "
          . $Wizard{$Name}{'MeleeScore'} . "\n"
          . "Battles survived: "
          . $Wizard{$Name}{'Battles'} . "\n"
          . "Status:           " );
    ...
    if ( $Wizard{$Name}{'Retired'} eq 1 ) {
        ...
        if ( $game =~ s/^N(.*)$/$1/ ) {
            ...
            if ( !&GetGameInfo( $game, $Name ) )    #Get %Info and $Turn
            {
                ...
                $String .=
                  "(Janitor has been notified: please don't use this mage!)\n";
                print STDERR
"Database inconsistency detected during INFO command processing: \n $Name in game $game\n";
            }
            else {
    ...
    foreach $wiz ( sort wizard_by_score @WizardNames ) {
        if ( !$Wizard{$wiz}{'Dead'} ) {
            if ( $Wizard{$wiz}{'Busy'} ) {
                ...
                $Entry =
                  sprintf( " %5s ", $Wizard{$wiz}{'Busy'} . $OnVacation );
            }
            else {
                $Entry = "       ";
            }
...
";
    ...
    foreach $wiz_index ( sort dead_by_score ( 0 .. $#DeadNames ) ) {
        $Scores .= sprintf( "%5s   %-18s %s\n",
            $DeadScores[$wiz_index], $DeadNames[$wiz_index],
            $DeadUsers[$wiz_index] );
        print HISTORY
"$DeadNames[$wiz_index] $DeadScores[$wiz_index] $DeadUsers[$wiz_index]\n";
    }
    close HISTORY;
